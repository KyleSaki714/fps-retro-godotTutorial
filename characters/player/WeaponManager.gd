...
func update_animation(velocity: Vector3, grounded: bool):
	# play idle if a weapon is currently being used, or we're in the air, or not fast enough
	if cur_weapon.has_method("is_idle") and !cur_weapon.is_idle():
		anim_player.play("idle")
	if !grounded or velocity.length() < 14.0:
...
func get_pickup(pickup_type, ammo):
	print("get_pickup(): pickup_type: ", pickup_type, " ammo: ", ammo)
	match pickup_type:
