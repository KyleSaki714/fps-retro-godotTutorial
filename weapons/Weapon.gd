...
func attack(attack_input_just_pressed : bool, attack_input_held: bool):
	...
	anim_player.stop()
	# if you don't stop the player while it's still playing, it'll just continue to play
	# play() does not reset the player
...
# returns true if there are no weapons currently being fired, 
# otherwise false
func is_idle():
